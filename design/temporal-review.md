...
- [x] **Drop the `SetWorkspace` call from `ChatCompletionActivity`** (finding 1).
      Done: it takes a throwaway workspace of its own and deletes it on the way
      out, since it needs the tool schemas rather than the tools and never
      writes anything. `CompletionRequest.WorkspaceDir` went with it.
...
- [x] **Mark permanent failures non-retryable** (finding 3).
      ...
      of `json.Unmarshal` and `(Workspace).resolve`. The argument errors are classified in
