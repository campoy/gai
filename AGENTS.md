...
`cutPoint` decides where. **The cut always lands on a user message**, because the API rejects a history whose tool result has no call, or whose `tool_calls` go unanswered, and tool traffic never spans two user messages. That rule is the load-bearing part of this file — `agent/compact_test.go` pins it down, along with a validator that rejects a history the API would. Don't relax it to drop "just a bit more"; a bad cut doesn't degrade the agent, it breaks the request outright.
