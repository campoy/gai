...
A workspace is passed around as a `*tools.Workspace`, and the four file tools built from one share that pointer. Both constructors therefore report failure as `nil, err` rather than a value that still looks usable, and a field that has to be shared — a lock, a cleanup handle — could be added later without four copies drifting apart. Nothing in it is mutable today, so nothing needs a lock today. A nil workspace, and one carrying no directory, are both refused by `resolve` rather than joined against, because `filepath.Join("", p)` is `p` — a path relative to the process's working directory, which is the repository `gai` is running in. `TestWorkspacelessToolsReachNothing` drives all four tools down both of those paths.
