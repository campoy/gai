...
The file tools work in a temporary workspace, not the repository. A `tools.Workspace` is a directory the file tools are confined to, and it is **passed to them at construction** — `tools.All(client, workspace)` builds each one around it, the same way `NewWebSearch(client)` closes over its client. `tools.NewWorkspace` creates a fresh temporary one and returns it with a cleanup function; `main` calls it once per run on the local path, and each eval case calls it per run, deferring the cleanup, so the directory and everything in it is deleted on exit. `tools.OpenWorkspace(dir)` takes a directory the caller names instead, creating it if needed and leaving its lifetime to the caller — that is what the Temporal tool activity uses.

The workspace used to be a package-level string set by a `SetWorkspace` initializer. Don't put it back: two agents in one process — two Temporal activities on one worker — then race on that string, and a tool call from one run resolves its path against the other's directory. The sandbox still holds in that state, but it guards the wrong directory. `TestWorkspacesAreIndependent` in `tools/file_test.go` pins it down. A nil `*Workspace` is legal and means "schemas only": the file tools are still advertised, but refuse every path. `ChatCompletionActivity` uses that, since it needs the tool schemas and never touches a file.

Every file tool routes its path through `(*Workspace).resolve` in `tools/file.go`, which rejects absolute paths and anything escaping the workspace. The model picks these paths out of untrusted text — never add a file tool that bypasses `resolve`, and keep the tests in `tools/file_test.go` passing.
