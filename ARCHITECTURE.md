...
| The workflow path emits no spans | `temporal/temporal.go:4–19` | No `agent`, LLM or tool spans, so neither Jaeger nor the evals can see a workflow run. The second execution path is entirely unmeasured. | known |
