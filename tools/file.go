...
//
// A Workspace is handed around as a pointer, and the four file tools built
// from one all close over that same pointer. That is what lets NewWorkspace
// and OpenWorkspace report a failure as (nil, err) instead of a value that
// still looks usable, and it leaves room for a field that has to be shared —
// a lock, a cleanup handle, a lazily created directory — without four copies
// drifting apart. Nothing in here is mutable today, so nothing needs a lock
// today.
//
// A nil *Workspace, and one carrying no directory, are both unusable: the file
// tools refuse every path rather than falling back on the process's working
...
// workspace or one that carries none.
